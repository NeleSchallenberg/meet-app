...
describe('<NumberOfEvents /> component', () => {
  ...
  test('render number input field', () => {
    expect(NumberOfEventsWrapper.find('input.amount')).toHaveLength(1);
  });

  test('render 32 as default value for number of events', () => {
    expect(NumberOfEventsWrapper.find('input.amount').prop('type')).toBe('number');
  ...
  test('render user input correctly', () => {
    ...
    expect(NumberOfEventsWrapper.find('input.amount').prop('value')).toBe(query);
  });

  test('change state when number input changes', () => {
    expect(NumberOfEventsWrapper.state('default')).toBe(32);
    const inputValue = { target: { value: 10 }};
    NumberOfEventsWrapper.find('input.amount').simulate('change', inputValue);
