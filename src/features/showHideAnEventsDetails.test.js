...
defineFeature(feature, (test) => {
	...
	test("An event element is collapsed by default", ({
		...
		then(
			"the user should see all event elements without their details",
			() => {
				AppWrapper.update();
				expect(AppWrapper.find(".event .event-details")).toHaveLength(
					0
				);
			}
		);
	...
	test("User can expand an event to see its details", ({
		...
		then(
			"the user should see the event element expanding and showing the events details",
			() => {
				AppWrapper.update();
				expect(AppWrapper.find(".event .event-details")).toHaveLength(
					1
	...
	test("User can collapse an event to hide its details", ({
		...
		then(
			"the user should see the event collapsing and hiding the details",
			() => {
				AppWrapper.update();
				expect(AppWrapper.find(".event .event-details")).toHaveLength(
					0
